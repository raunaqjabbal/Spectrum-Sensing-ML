...
class Classification:
    ...
    def __init__(self, X_train=None,y_train=None,X_test=None,y_test=None,Samples=50,SU=3,X_test_2=None,SNR=None,Slots=1):
    ...
    def Logistic(self):
        ...
        type='Logistic'
        marker="o"
        parameters =[{'C': [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4], 'max_iter':[5000],'solver': ['newton-cg','lbfgs','sag'], 'penalty': ['l2']},
                     {'C': [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4], 'max_iter':[5000], 'solver': ['saga','liblinear'], 'penalty': ['l1','l2']}]
    ...
    def OR(self):
        ...
        for i in range(len(Pfa_target)):
            ...
            # lambd = 2*special.gammainccinv(Samples//2,Pfa_target[i])/Samples
    ...
    def AND(self):
        ...
        for i in range(len(Pfa_target)):
            ...
            # lambd = 2*special.gammainccinv(Samples//2,Pfa_target[i])/Samples
            lambd = 2*special.gammainccinv(self.SU/2,Pfa_target[i])/self.SU
            y_pred=np.array(np.sum(self.X_test>=lambd,1)==self.SU*self.Slots, dtype=int)
